    ...
    std::thread::spawn(move || loop {
        ...
        let lowest_wait = match inner.client.poll(now) {
            ...
            TurnPollRet::AllocateTcpSocket {
                ...
                let tcp = TcpStream::connect_timeout(&peer_addr, std::time::Duration::from_secs(5))
                    .inspect_err(|e| {
                ...
                inner.client.allocated_tcp_socket(
                    id,
                    socket,
                    peer_addr,
                    local_addr,
...
impl<T: AsRef<[u8]> + std::fmt::Debug> Client<T> for ClientTcp {
    ...
    fn create_permission(
        ...
        let mut inner = self.inner.0.lock().unwrap();
        inner.client.create_permission(transport, peer_addr, Instant::from_std(self.base_instant))?;
        self.inner.1.notify_one();
        Ok(())
    }
    ...
    fn tcp_connect(&self, peer_addr: SocketAddr) -> Result<(), TcpConnectError> {
        let mut inner = self.inner.0.lock().unwrap();
        inner.client.tcp_connect(peer_addr, Instant::from_std(self.base_instant))
    }
}
